...
describe('dataImportController test', () => {
  ...
  describe('POST /api/upload', () => {
    it('should  return a response of 400 if a csv with missing column is uploaded', (done) => {
      request(app)
        ...
        .attach(
          'data',
          path.resolve(__dirname, './csvData/data.missing.sample.csv')
        )
        .expect(400)
        .end(done);
    });
    ...
    it('should return a response of 400 if a csv with missing mandatory column is uploaded', (done) => {
      request(app)
        ...
        .attach(
          'data',
          path.resolve(__dirname, './csvData/column.missing.sample.csv')
        )
        .expect(400)
        .end(done);
    });
    ...
    it('should return a response of 400 if a csv with invalid column name is uploaded', (done) => {
      request(app)
        ...
        .attach(
          'data',
          path.resolve(__dirname, './csvData/invalid.column.name.csv')
        )
        .expect(400)
        .end(done);
    });
    ...
    it('it should be able to upload the excel data and return a response of 204', (done) => {
      request(app)
        .post('/api/upload')
        .attach('data', path.resolve(__dirname, './csvData/data.sample.csv'))
        .expect(204)
        .end(done);
    });
  });
  ...
  describe('PUT api/upload', () => {
    ...
    it('should update the teacher"s column correctly', async (done) => {
      await request(app)
        .put('/api/upload')
        .attach('data', path.resolve(__dirname, './csvData/data.update.csv'))
        .expect(204);
